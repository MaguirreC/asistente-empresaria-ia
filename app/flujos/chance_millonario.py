"""Flujo guiado para armar una apuesta de Chance Millonario.

Recoge, en este orden:

    lotería 1 -> lotería 2 -> número 1 -> número 2 -> número 3 -> número 4 -> número 5

y termina devolviéndole al front un formulario con todo listo para mapear.

Es de la misma familia que Doble Play (`knowledge/otras-modalidades.md`,
`knowledge/chance-millonario.md`): se eligen **2 loterías** de las que juegan
hoy y **5 números de 4 cifras**, con la expectativa de que 2 de esos números
coincidan con el resultado de las loterías elegidas. El valor **no se
pregunta**: es fijo, $6.000, a diferencia del chance tradicional.

Las loterías salen de un endpoint propio (`tools/loterias_acumulados.py`),
distinto del de chance tradicional: no recibe fecha (el backend siempre
devuelve las de HOY), así que este flujo no tiene paso de fecha.
"""
import re
...
from app.tools.loterias_acumulados import loterias_chance_millonario

# Fijo, no se pregunta — a diferencia del chance tradicional.
VALOR_APUESTA = 6_000
...
def _loterias_disponibles() -> list:
    """Loterías vigentes hoy, o se corta el flujo con un mensaje entendible."""
    loterias = loterias_chance_millonario()
    if loterias is None:
        raise FlujoNoDisponible(
            "No pude consultar las loterías de Chance Millonario en este "
            "momento. 🙏 Inténtalo de nuevo en un rato, o ármalo directamente "
            "en la pantalla."
        )
    if not loterias:
        raise FlujoNoDisponible(
            "Hoy ya no hay loterías disponibles para Chance Millonario. "
            "Escríbeme de nuevo mañana."
        )
    return loterias
...
def _paso_primera_loteria(datos: dict) -> Paso:
    ...
    return paso_loteria(
        loterias,
        datos.get("jornada_1"),
        pregunta="Vamos a armar tu Chance Millonario. 💰\n\n¿Cuál es la primera lotería que quieres jugar?",
        id="loteria_1",
    )


def _paso_segunda_loteria(datos: dict) -> Paso:
    # La segunda lotería no puede repetir la primera: son 2 loterías
...
def _resumen(datos: dict) -> str:
    ...
    return (
        "¡Listo! Así queda tu Chance Millonario:\n\n"
        f"• **Loterías:** {datos['loteria_1']['nombre']} y {datos['loteria_2']['nombre']}\n"
        f"• **Números:** {numeros}\n"
        f"• **Valor:** {formatear_pesos(VALOR_APUESTA)}\n\n"
        "Te lo dejo cargado en la pantalla de Chance Millonario para que lo "
        "revises y confirmes la compra. 👇"
    )
